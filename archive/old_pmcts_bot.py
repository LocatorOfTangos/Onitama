...
from opponents import ab_search_bot
...
class MCTSBot:
    def __init__(self, time_limit=1.0, temperature=OPT_TEMP, exploration=OPT_EXPLORE, num_threads=2):
        """Initialize MCTS bot.
    ...
    def request_move(self, board, time_limit=None):
        ...
        # Use ThreadPoolExecutor for parallel rollouts
    ...
    def _mcts_iteration(self, root, deadline):
        ...
        # Expansion: lock only when modifying untried_moves/children
        ...
        # Simulation
        if node.is_terminal():
            # If node is terminal, do not rollout.
            reward = self.evaluator._evaluate_position(node.board)
        else:
            start_rollout = time.time()
            reward = self._rollout(node.board, deadline)
            self.rollout_time += time.time() - start_rollout
        
        # Backpropagation: update values and visit counts up the tree with per-node locking
